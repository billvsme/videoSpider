...
def get_celebrity_info(info_node_dict):
    ...
    if '性别' in info_node_dict:
        celebrity_info['sex'] = info_node_dict['性别'].span.next_sibling.string.strip(': \n')
    if '星座' in info_node_dict:
        celebrity_info['constellation'] = info_node_dict['星座'].span.next_sibling.string.strip(': \n')
    if '出生日期' in info_node_dict:
        celebrity_info['birthday'] = info_node_dict['出生日期'].span.next_sibling.string.strip(': \n')
    if '出生地' in info_node_dict:
        celebrity_info['born_place'] = info_node_dict['出生地'].span.next_sibling.string.strip(': \n')
    ...
    if 'imdb编号' in info_node_dict:
        celebrity_info['imdb_number'] = info_node_dict['imdb编号'].find('a').string
